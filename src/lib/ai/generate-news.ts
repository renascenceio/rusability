...
function buildTimelinessPolicy(now: Date): string {
  ...
  return [
    ...
    `При СОМНЕНИИ между «в/за прошлый период» и точкой отсчёта — выбирай "stale". Любые «итоги», «результаты», «за N месяцев ${y - 1}/${y - 2} года», «выдали/выросли/составили … в ${y - 1}/${y - 2} году» как ГЛАВНАЯ тема заголовка → всегда "stale". Лента должна быть строго свежей.`,
