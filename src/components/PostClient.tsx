...
import { type Article, ARTICLES, INDUSTRY_TOOLS} from"@/lib/data";
...
interface PostClientProps {
  ...
  relatedNews: { id: string | number; title: string; category: string }[];
}

export default function PostClient({ displayData, authorArticles, relatedNews }: PostClientProps) {
 ...
 <p className="text-lg font-bold text-[var(--foreground)] leading-tight">{displayData.author}</p>
          ...
          <div className="hidden md:block h-10 w-px bg-[var(--foreground)]/10" />

          <div className="flex items-center gap-4">
            <button className="hig-button-primary py-2 px-6 text-sm flex items-center gap-2">{t("article.follow")}</button>
      ...
      <div className="relative mb-20 rounded-[40px] overflow-hidden shadow-2xl h-[400px] md:h-[600px]">
        <Image
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-[120px,1fr,320px] gap-10 md:gap-20">
        ...
        <article className="flex-1 space-y-10 max-w-2xl mx-auto lg:mx-0">
          <p className="text-xl md:text-2xl font-medium leading-relaxed text-[var(--foreground)] italic pl-8 py-2 border-l-4 border-hig-blue">
          ...
          <p className="text-[var(--foreground)] body-serif text-lg leading-relaxed">
          ...
          <h2 className="text-3xl font-bold tracking-tight pt-10 text-[var(--foreground)]">{t("article.tool")} Approach to Empathy</h2>
          ...
          <div className="bg-hig-blue/5 p-12 rounded-[32px] space-y-6">
            <h3 className="text-xl font-bold uppercase tracking-widest text-hig-blue">{t("article.keyInsights")}</h3>
            <ul className="space-y-4">
              {[
                ...
                <li key={i} className="flex gap-4 items-start">
                  ...
                  <span className="text-lg font-medium text-[var(--foreground)] leading-tight">{item}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="space-y-6 pt-12">
            <h4 className="text-xs font-bold uppercase tracking-widest text-secondary">{t("article.related")}</h4>
          ...
          <div className="pt-20 space-y-12">
            <h3 className="text-2xl font-bold tracking-tight text-[var(--foreground)]">{t("article.moreFrom")} {displayData.author}</h3>
              ...
              {(authorArticles.length > 0 ? authorArticles : ARTICLES.slice(3, 5)).map(a => (
                <Link key={a.id} href={`/posts/${a.id}`} className="hig-card p-8 group">
                  <div className="space-y-4">
                    ...
                    <div className="inline-flex items-center gap-2 text-sm font-bold text-hig-blue pt-4">{t("article.readNow")} <ArrowUpRight className="w-4 h-4" /></div>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <aside className="space-y-12 sticky top-32 h-fit">
          <div className="hig-card p-8">
            <h4 className="text-[10px] font-bold uppercase tracking-widest text-secondary mb-8 pb-4 flex items-center gap-2 border-b border-[var(--foreground)]/5">
              ...
              {relatedNews.map(news => (
                <div key={news.id} className="group cursor-pointer">
                  ...
                  <div className="mt-4 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="text-[10px] text-secondary font-bold uppercase">{t("article.briefing")}</span>
          ...
          <div className="hig-card p-8 bg-hig-blue text-white relative overflow-hidden shadow-lg shadow-hig-blue/20">
            <div className="relative z-10 space-y-4">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-white/60">{t("article.proPerspective")}</h4>
              <p className="font-bold text-sm leading-relaxed">
                Get the full analysis on how privacy tracking will impact your Q4 ROI.
              </p>
              <button className="bg-white text-hig-blue text-xs font-black uppercase tracking-widest px-4 py-2 rounded-full w-full">{t("article.unlockNow")}</button>
            </div>
