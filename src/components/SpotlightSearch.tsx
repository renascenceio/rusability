...
export const SpotlightSearch = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4">
      ...
      >
        <div className="flex items-center p-6 gap-4 border-b border-[var(--border)] dark:border-[var(--border)]">
          <Search className="w-6 h-6 text-[var(--foreground)] dark:text-[var(--foreground)]" />
          <input
            autoFocus
            className="flex-1 bg-transparent border-none outline-none text-2xl text-black dark:text-white placeholder:text-[var(--foreground)]/40 dark:placeholder:text-[var(--foreground)]/40 font-black"
                  ...
                  {[
                    ...
                    <button key={item.title} className="flex items-center gap-4 p-4 text-left hover:bg-[var(--muted)] rounded-2xl transition-all group border border-transparent hover:border-[var(--border)]">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-[var(--foreground)] font-bold uppercase tracking-widest">{item.desc}</span>
                      </div>
                    </button>
                  ))}
                </div>
