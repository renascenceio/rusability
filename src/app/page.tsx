...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 md:py-16 space-y-16 md:space-y-24">
      ...
      <section className="relative group overflow-hidden rounded-[32px] md:rounded-[48px] border border-black/5 dark:border-black/5 bg-white dark:bg-white shadow-sm">
        <Link href={`/posts/${featuredArticle.id}`} className="grid grid-cols-1 lg:grid-cols-2 min-h-[500px] md:min-h-[600px]">
          ...
          <div className="p-8 md:p-12 lg:p-16 flex flex-col justify-center space-y-6 md:space-y-8">
            ...
            <p className="text-base md:text-xl text-black/90 dark:text-black/90 leading-relaxed line-clamp-3 font-black">
              {featuredArticle.excerpt}
            </p>
            <div className="flex items-center justify-between pt-4 md:pt-8 border-t border-black/5 dark:border-black/5">
               <div className="flex items-center gap-4">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white dark:bg-white flex items-center justify-center">
                    <User className="w-5 h-5 md:w-6 md:h-6 text-black/80" />
                  </div>
                  <div className="flex flex-col">
                    <span className="font-bold text-sm md:text-base text-black dark:text-white">{featuredArticle.author}</span>
                    <span className="text-xs text-black/80 dark:text-black/90 font-black tracking-widest uppercase">{featuredArticle.category} • {featuredArticle.time}</span>
                  </div>
               </div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-3 border-b border-black/5 dark:border-black/5 pb-4">
          <Users className="w-5 h-5 text-hig-blue" />
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-black dark:text-zinc-100">Voices to follow</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {POPULAR_AUTHORS.map((author) => (
            <Link key={author.id} href={`/profile/${author.id}`} className="hig-card p-6 flex items-center gap-6 group hover:bg-hig-blue/[0.02] bg-white dark:bg-white/50 border-black/5 dark:border-black/5 shadow-none hover:border-black/5">
              <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-white dark:border-black/5 shadow-md ring-1 ring-black/5 dark:ring-zinc-800 group-hover:ring-hig-blue/30 transition-all">
                <Image src={author.avatar} alt={author.name} fill className="object-cover" />
              </div>
              <div className="space-y-1">
                <h4 className="font-black text-black dark:text-white group-hover:text-hig-blue transition-colors">{author.name}</h4>
                <p className="text-xs text-black/90 dark:text-black/90 font-black uppercase tracking-widest">{author.role}</p>
        ...
        <div className="space-y-16 md:space-y-24">
          <section className="space-y-10 md:space-y-16">
             <div className="flex items-center justify-between border-b border-black/5 dark:border-black/5 pb-6">
                <div className="space-y-1">
                   <h2 className="text-2xl md:text-3xl font-black tracking-tight text-black dark:text-white italic">Human stories and ideas</h2>
                   <p className="text-xs text-black/80 dark:text-black/90 font-black uppercase tracking-widest">The rusability perspective</p>
                </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 items-stretch">
               ...
               <Link href={`/posts/${otherArticles[0].id}`} className="md:col-span-2 group flex flex-col space-y-6">
                 <div className="relative flex-1 min-h-[300px] md:min-h-[440px] rounded-[32px] overflow-hidden border border-black/5 dark:border-black/5">
                 ...
                 <div className="space-y-4">
                    <div className="flex items-center gap-3 text-xs font-black uppercase tracking-[0.2em] text-hig-blue">
                       ...
                       <span className="text-black/80 dark:text-black/90">{otherArticles[0].time}</span>
                    </div>
                    <h3 className="text-3xl md:text-4xl font-black leading-tight group-hover:text-hig-blue transition-colors text-black dark:text-white">
                       {otherArticles[0].title}
                    </h3>
                    <p className="text-black/90 dark:text-black/90 leading-relaxed font-black line-clamp-2">
               ...
               <Link href={`/posts/${otherArticles[1].id}`} className="md:col-span-1 group flex flex-col space-y-6">
                 <div className="relative flex-1 min-h-[300px] rounded-[32px] overflow-hidden border border-black/5 dark:border-black/5">
                 ...
                 <div className="space-y-4">
                    ...
                    <h3 className="text-xl md:text-2xl font-black leading-tight group-hover:text-hig-blue transition-colors text-black dark:text-white">
                       {otherArticles[1].title}
                    </h3>
                    <p className="text-sm text-black/90 dark:text-black/90 font-black line-clamp-2 leading-relaxed">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
               ...
               {otherArticles.slice(2).map((article) => (
                 <Link key={article.id} href={`/posts/${article.id}`} className="group flex flex-col space-y-6 h-full">
                   <div className="relative aspect-[4/3] rounded-[32px] overflow-hidden border border-black/5 dark:border-black/5 shrink-0">
                   ...
                   <div className="space-y-3 flex-1">
                     <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] text-hig-blue">
                        ...
                        <span className="w-1.5 h-1.5 rounded-full bg-white" />
                        <span className="text-black/80 dark:text-black/90">{article.time}</span>
                     </div>
                     <h3 className="text-xl font-black leading-tight group-hover:text-hig-blue transition-colors line-clamp-2 text-black dark:text-white">{article.title}</h3>
                     <p className="text-sm text-black/90 dark:text-black/90 font-black line-clamp-2 leading-relaxed">{article.excerpt}</p>
                   </div>
                 </Link>
               ))}
             </div>
          ...
          <section className="bg-white dark:bg-black border border-black/5 dark:border-black/5 rounded-[32px] md:rounded-[48px] p-8 md:p-16 text-black dark:text-white overflow-hidden relative shadow-none">
            <div className="relative z-10">
               <div className="flex items-center gap-3 mb-10 text-hig-blue font-black uppercase tracking-[0.2em] text-[10px]">
                  <Sparkles className="w-4 h-4" />
                  <span>EMOTIONAL INTELLIGENCE</span>
               </div>
               <h2 className="text-3xl md:text-5xl font-black mb-12 tracking-tight leading-tight text-black dark:text-white">Crafting the future of marketing with heart.</h2>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                  {INDUSTRY_TOOLS.map((tool) => (
                    <Link key={tool.id} href={tool.link} className="hig-card bg-white dark:bg-white/10 border-black/5 dark:border-white/10 hover:bg-white p-6 space-y-6 block group shadow-none">
                       <div className="relative w-full aspect-square rounded-[24px] overflow-hidden border border-black/5">
                       ...
                       <div className="space-y-2">
                          ...
                          <p className="text-xs text-black/90 dark:text-black/70 font-black leading-relaxed">{tool.description}</p>
                       </div>
                       <div className="pt-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
          ...
          <div className="hig-card p-8 bg-white dark:bg-white/50 border-black/5 dark:border-black/5 shadow-none border-b-2">
             <div className="flex items-center gap-2 mb-8 border-b border-black/5 dark:border-black/5 pb-4">
                ...
                <h3 className="font-black text-xl text-black dark:text-white uppercase tracking-tight">Pulse News</h3>
             </div>
             <div className="space-y-6">
                {INDUSTRY_NEWS.map((item) => (
                  <div key={item.id} className="group cursor-pointer space-y-2 border-b border-black/5 dark:border-black/5 pb-4 last:border-0">
                    ...
                    <h4 className="text-sm font-black leading-snug text-black dark:text-zinc-100 group-hover:text-hig-blue transition-colors">{item.title}</h4>
                  </div>
                ))}
             </div>
          </div>
          ...
          <div className="hig-card p-8 bg-white dark:bg-white/50 border-black/5 dark:border-black/5 shadow-none border-b-2">
            <div className="flex items-center gap-2 mb-8 border-b border-black/5 dark:border-black/5 pb-4">
              ...
              <h3 className="font-black text-xl text-black dark:text-white uppercase tracking-tight">Trending Vibes</h3>
            </div>

            <div className="space-y-8">
              {TRENDING.map((tag, i) => (
                <Link key={tag} href={`/news?q=${tag}`} className="flex gap-6 group">
                  <span className="text-4xl font-black text-black/70 dark:text-zinc-800 group-hover:text-hig-blue transition-colors shrink-0">0{i+1}</span>
                  <div className="space-y-2 pt-1">
                    <h4 className="font-black text-sm leading-snug text-black dark:text-zinc-200 group-hover:text-hig-blue transition-colors line-clamp-3 uppercase tracking-tight">
                      {tag}
                    </h4>
                    <p className="text-[8px] text-black dark:text-black/90 uppercase font-black tracking-[0.2em] flex items-center gap-1.5">
                       <span className="w-2 h-2 rounded-full bg-emerald-600" />
                       <span className="text-black font-black">LIVE INSIGHT</span>
                    </p>
                  </div>
                </Link>
              ))}
