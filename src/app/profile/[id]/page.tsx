...
import { User, Settings, Sparkles, Heart, Clock, Sliders, ChevronRight, TrendingUp, Users, Target, Edit3, BookOpen, MessageCircle, FileText, Trash2, Shield, Bell, ArrowRight, X, Search as SearchIcon, Plus } from "lucide-react";
...
export default function ProfilePage() {
 ...
 { id: 1, title:"The Shift in SEO Strategy for 2024", time:"5 min read", cat:"Search Strategy", date:"Saved 2 days ago"},
 { id: 2, title:"Generative AI for Media Outreach", time:"8 min read", cat:"Public Relations", date:"Saved 1 week ago"},
 { id: 3, title:"Brand Safety in the Age of LLMs", time:"12 min read", cat:"Brand Protection", date:"Saved 2 weeks ago"},
 ]);
 ...
 <div className="p-12 text-center bg-[var(--muted)]/50 rounded-[32px] space-y-4">
 <Heart className="w-12 h-12 text-tertiary mx-auto opacity-20" />
 <p className="text-secondary font-medium italic">No saved articles yet. Bookmark articles you want to read later and they&apos;ll appear here.</p>
 </div>
 ) : (
 savedArticles.map((article) => (
 <div key={article.id} className="hig-card p-6 flex items-center justify-between group">
 <Link href={`/posts/${article.id}`} className="space-y-2 flex-1">
 ...
 <button
 onClick={() => setSavedArticles(prev => prev.filter(a => a.id !== article.id))}
 className="p-3 text-rose-500 bg-rose-500/10 rounded-xl md:opacity-0 group-hover:opacity-100 transition-all hover:scale-110"
 title="Remove"
 >
 <Trash2 className="w-4 h-4" />
 </button>
 <Link href={`/posts/${article.id}`} className="p-3 text-secondary hover:text-hig-blue bg-[var(--muted)] rounded-xl transition-all">
 ...
 <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-tertiary" />
 <input
 type="text"
 placeholder="Filter topics..."
 className="w-full bg-[var(--muted)] rounded-xl py-2.5 pl-10 pr-4 text-[10px] font-bold uppercase tracking-widest outline-none focus:ring-2 focus:ring-hig-blue/10"
 />
 </div>

 <div className="flex flex-wrap gap-2">
  {interests.map((tag) => (
 <span key={tag} className="bg-[var(--muted)] px-3 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest text-hig-blue flex items-center gap-2 group/tag hover:bg-hig-blue/5 transition-all">
 {tag}
 <button
 ...
 <p className="text-[9px] font-black uppercase tracking-[0.2em] text-tertiary">Suggested Topics</p>
 <div className="flex flex-wrap gap-2">
 {["Privacy","Analytics","Social Media","Email Marketing","B2B","E-commerce"].filter(t => !interests.includes(t)).map(tag => (
 <button
 key={tag}
 onClick={() => setInterests(prev => [...prev, tag])}
 className="bg-[var(--muted)]/50 px-3 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest text-secondary hover:text-hig-blue hover:bg-hig-blue/5 transition-all flex items-center gap-1.5"
 >
